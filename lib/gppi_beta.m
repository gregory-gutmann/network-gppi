...
for r=1:length(roiList)
    ...
    try
        load([ntwFolder, 'eigenvariates\SPM.mat'])
        SPMPPI.swd          = [ntwFolder, '\gppi\estimates\',roi,'\'];
        ...
        for sess=1:numel(SPM.Sess)
            load([ntwFolder, 'gppi\parameters\PPI_', roi, '_sess_', num2str(sess), '.mat']);
        ...
        %{
        V                   = spm_vol([ntwFolder, 'roi_masks\', roiFile]);
        SPM.xM.VM           = V;
        %}
...
save([ntwFolder, 'gppi\estimates\errorRoisBeta'],'errorRois');
