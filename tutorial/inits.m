...
input. The nifti files are also decompressed first-level activity analysis
is conducted.
...
sphere          = load([wkdir,'\tutorial\templates\sphere_8mm.mat']).sphereCutXYZ;
sphereInfo      = spm_vol([wkdir,'\tutorial\templates\sphereInfo.nii']);
...
evGLM.tunits    = ['secs'];
evGLM.sub       = sub;
evGLM.folder    = {[wkdir, '\tutorial\data\firstlevel\', sub, '\']};
evGLM.spmmat    = {[wkdir, '\tutorial\data\firstlevel\', sub, '\SPM.mat']};
...
evGLM.prepro(1)= {[wkdir,'\tutorial\data\preprocessed\', sub, '_preproc1.nii']};
evGLM.prepro(2)= {[wkdir,'\tutorial\data\preprocessed\', sub, '_preproc2.nii']};
...
%% Save workspace
save([wkdir,'\tutorial\parameters.mat'])
